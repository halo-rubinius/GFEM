...
int main()
{
    ...
    // 添加一个三角形单元
    mesh.addElement(
        Element(0, {0, 1, 2}, std::make_shared<Tri3ShapeFunction>()));

    // 添加一个四边形单元
    mesh.addElement(
        Element(1, {0, 1, 2, 3}, std::make_shared<Quad4ShapeFunction>()));
    ...
    // 生成全局自由度编号
    std::cout << "Total Degrees of Freedom: " << mesh.generateGlobalDOFs()
              << std::endl;
